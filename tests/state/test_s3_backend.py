...
def test_persistent_cas_conflict_surfaces(aws: None, monkeypatch: pytest.MonkeyPatch) -> None:
    """A compare-and-swap that keeps losing must surface, never silently clobber."""
    backend = _backend()
    ...
    with pytest.raises(StateError) as exc:
        backend.put(node("a"))
    assert "kept changing under this run" in str(exc.value)
...
def test_probe_recovers_from_a_leftover_scratch_key(aws: None) -> None:
    ...
    key behind; every later probe would lose its first conditional put to it
    forever and warn on a healthy endpoint."""
    client: Any = boto3.client("s3", region_name=REGION)
    client.put_object(Bucket=BUCKET, Key=f"{KEY}.atlantide-probe", Body=b"stale")

    result = _backend().probe()
    assert result.status == "ok"
    keys = {o["Key"] for o in client.list_objects_v2(Bucket=BUCKET).get("Contents", [])}
    assert f"{KEY}.atlantide-probe" not in keys  # cleaned up again
...
def test_a_write_that_raced_this_run_is_rebased_not_clobbered(aws: None) -> None:
    """A lost compare-and-swap is rebased onto the fresh document.

    Per-node leasing deliberately lets two runs over disjoint subgraphs share
    the one state object, so another run moving the object on is not an error.
    What must hold is that neither run's rows are lost: this run's next write
    re-reads, re-applies its own mutation, and preserves the racing writer's.
    """
    backend = _backend()
    assert is_successful(backend.acquire_lock("me", 300.0, frozenset({"a", "b"})))
    backend.put(node("a"))

    _backend().put(node("rogue"))  # a different run writes the object
    assert is_successful(backend.renew_lock("me", 300.0, frozenset({"a", "b"})))

    backend.put(node("b"))
    committed = set(_backend().load().nodes)
    assert committed == {"a", "b", "rogue"}, "both runs' writes must survive"


def test_acquiring_a_lock_still_drops_a_stale_view(aws: None) -> None:
    """The counterpart: acquisition must keep invalidating, or a run would
    compare-and-swap against an ETag from before it held the lock."""
    backend = _backend()
    backend.load()  # populate the cache from a pre-lock read
    other = _backend()
    other.put(node("a"))  # a different run moves the object on
...
def test_transient_transaction_cancel_does_not_lose_the_lease(
    aws: None, monkeypatch: pytest.MonkeyPatch
) -> None:
    """DynamoDB cancels transactions for transient reasons (item conflicts,
    throttling) too; only a ConditionalCheckFailed means another owner holds the
    scope. A renew hitting a transient cancel must retry and succeed — treating
    it as contention cancelled a healthy run mid-apply."""
    backend = _backend()
    assert is_successful(backend.acquire_lock("me", 300.0, frozenset({"a"})))
    ...
    assert is_successful(backend.renew_lock("me", 300.0, frozenset({"a"})))


def test_unreadable_holds_fail_closed(aws: None, monkeypatch: pytest.MonkeyPatch) -> None:
    """Holds DynamoDB keeps returning as unprocessed must not read as unheld —
    that would wave a fenced write through a lock another run owns."""
    backend = _backend()
    lock = backend.acquire_lock("me", 300.0, frozenset({"a"}))
    assert is_successful(lock)
    backend.bind_lease(lock.unwrap())  # fence writes, as with_lock does
    ...
    with pytest.raises(StateError, match="unprocessed"):
        backend.put(node("a"))

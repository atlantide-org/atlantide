...
def ssm(monkeypatch: pytest.MonkeyPatch) -> Iterator[Any]:
    ...
    with mock_aws():
        ...
        client.put_parameter(Name=f"{PREFIX}db_password", Value="hunter2", Type="SecureString")
        yield client
...
def test_check_passes_if_the_probe_name_somehow_exists(ssm: Any) -> None:
    ssm.put_parameter(Name=f"{PREFIX}atlantide-preflight-probe", Value="x", Type="String")

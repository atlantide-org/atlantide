...
deterministic and independent of clone/checkout mechanics — the same content
always hashes the same, matching the byte-stable ethos of the IR hash. Derived
Python caches (``__pycache__``, ``*.pyc``) and the repo's ``.git`` are excluded so
they never move the hash.
"""
